...
def compute_statistics_jit(overlaps,
    ...
    dt_depths = dt_datas[:, 5]
    gt_depths = gt_datas[:, 5]
    ...
    tp, fp, fn, similarity, similarity_depth = 0, 0, 0, 0, 0
    # thresholds = [0.0]
    # delta = [0.0]
    ...
    for i in range(gt_size):
        ...
        if (valid_detection == NO_DETECTION) and ignored_gt[i] == 0:
            ...
            if compute_aos:
                ...
                delta_idx += 1
            if compute_ads:
                delta_depth[delta_idx_depth] = gt_depths[i] - dt_depths[det_idx]
                delta_idx_depth += 1
            assigned_detection[det_idx] = True
    if compute_fp:
        for i in range(det_size):
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
        if compute_ads:
            tmp = np.zeros((fp + delta_idx_depth, ))
            for i in range(delta_idx_depth):
                tmp[i + fp] = np.exp(-np.abs(delta_depth[i]))
            if tp > 0 or fp > 0: 
                similarity_depth = np.sum(tmp) 
            else:
                similarity_depth = -1
    return tp, fp, fn, similarity, similarity_depth, thresholds[:thresh_idx]
...
def fused_compute_statistics(overlaps,
    ...
    for i in range(gt_nums.shape[0]):
        for t, thresh in enumerate(thresholds):
            ...
            tp, fp, fn, similarity, similarity_depth, _ = compute_statistics_jit(
                overlap,
                gt_data,
                dt_data,
                ignored_gt,
                ...
                compute_ads=compute_ads)
            pr[t, 0] += tp
            pr[t, 1] += fp
            pr[t, 2] += fn
            if similarity != -1:
                pr[t, 3] += similarity
            if similarity_depth != -1:
                pr[t, 4] += similarity_depth
        gt_num += gt_nums[i]
        dt_num += dt_nums[i]
        dc_num += dc_nums[i]
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        ...
        gt_datas = np.concatenate(
            [gt_annos[i]['bbox'], gt_annos[i]['alpha'][..., np.newaxis], gt_annos[i]['location'][..., [2]]], 1)
        dt_datas = np.concatenate([
            dt_annos[i]['bbox'], dt_annos[i]['alpha'][..., np.newaxis], dt_annos[i]['location'][..., [2]],
...
def eval_class(gt_annos,
    ...
    recall = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    prs = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS, 5]
    )
    thrs = np.zeros(
        [num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS]
    )
    aos = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    ads = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit(
                        ...
                        thresh=0.0,
                        compute_fp=False)
                    tp, fp, fn, similarity, similarity_depth, thresholds = rets
                    thresholdss += thresholds.tolist()
                thresholdss = np.array(thresholdss)
                thresholds = get_thresholds(thresholdss, total_num_valid_gt)
                thresholds = np.array(thresholds)
                pr = np.zeros([len(thresholds), 5])  # tp, fp, fn, op, dp
                idx = 0
                for j, num_part in enumerate(split_parts):
                ...
                for i in range(len(thresholds)):
                    ...
                    precision[m, idx_l, k, i] = pr[i, 0] / (
                        pr[i, 0] + pr[i, 1])
                    prs[m, idx_l, k, i] = pr[i, :]
                    thrs[m, idx_l, k, i] = thresholds[i]
                    if compute_aos:
                        aos[m, idx_l, k, i] = pr[i, 3] / (pr[i, 0] + pr[i, 1])
                    if compute_ads:
                        ads[m, idx_l, k, i] = pr[i, 4] / (pr[i, 0] + pr[i, 1])
                for i in range(len(thresholds)):
                    precision[m, idx_l, k, i] = np.max(
                        precision[m, idx_l, k, i:], axis=-1)
                    recall[m, idx_l, k, i] = np.max(
                        recall[m, idx_l, k, :i + 1], axis=-1)
                    if compute_aos:
                        aos[m, idx_l, k, i] = np.max(
                            aos[m, idx_l, k, i:], axis=-1)
                    if compute_ads:
                        ads[m, idx_l, k, i] = np.max(
                            ads[m, idx_l, k, i:], axis=-1
                        )
    ret_dict = {
        'recall': recall,
        'precision': precision,
        'depth': ads,  # depth precision
        'orientation': aos,
        'tp': prs[..., 0],
        'fp': prs[..., 1],
        'fn': prs[..., 2],
...
def get_mAP(prec, n_sample_pts=40):
    sums = 0
    if n_sample_pts == 11:
        for i in range(0, prec.shape[-1], 4):
            sums = sums + prec[..., i]
        return sums / 11 * 100
    elif n_sample_pts == 40:
        for i in range(1, prec.shape[-1]):
            sums = sums + prec[..., i]
        return sums / 40 * 100
    else:
        raise ValueError("invalid argument! only 11 or 40 are supported.")
...
def do_eval(gt_annos,
    ...
    if 'bbox' in eval_types:
        ret = eval_class(
            ...
            compute_ads=('ads' in eval_types))
        # ret: [num_class, num_diff, num_minoverlap, num_sample_points]
        mAP_bbox = get_mAP(ret['precision'])
        if 'aos' in eval_types:
            mAP_aos = get_mAP(ret['orientation'])
        if 'ads' in eval_types:
            mAP_ads = get_mAP(ret['depth'])
        if out_dir is not None:
            report(out_dir, ret, current_classes, min_overlaps, 'bbox')
    ...
    if 'bev' in eval_types:
        ...
        if out_dir is not None:
            report(out_dir, ret, current_classes, min_overlaps, 'bev')
    ...
    if '3d' in eval_types:
        ...
        if out_dir is not None:
            report(out_dir, ret, current_classes, min_overlaps, '3d')
    return mAP_bbox, mAP_bev, mAP_3d, mAP_aos, mAP_ads
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
    ...
    mAP_bbox, mAP_bev, mAP_3d, mAP_aos, mAP_ads = do_eval(gt_annos, dt_annos,
...
def kitti_eval(gt_annos,
    ...
    name_to_class.update({v.lower(): n for n, v in class_to_name.items()})
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes]
    current_classes_int = []
    for curcls in current_classes:
        if isinstance(curcls, str) and curcls in name_to_class:
            current_classes_int.append(name_to_class[curcls])
        elif isinstance(curcls, int):
            current_classes_int.append(curcls)
        else: 
            raise RuntimeError("Not supported data type: {}. Int/Str expected".format(type(curcls)))
    ...
    if compute_ads:
        eval_types.append('ads')

    mAPbbox, mAPbev, mAP3d, mAPaos, mAPads = do_eval(gt_annos, dt_annos,
                                             current_classes, min_overlaps,
                                             eval_types, out_dir)
...
def report(out_dir, ret, current_classes, min_overlaps, eval_type):
    ...
    for class_id, class_name in enumerate(current_classes):
        for min_overlap_id, min_overlap in enumerate(min_overlaps):
            min_overlap = min_overlap[:, class_id]
            out_file = out_dir / f'{eval_type}-{class_id}-{min_overlap[0]:.1f},{min_overlap[1]:.1f},{min_overlap[2]:.1f}.png'
            plt.figure(figsize=(16, 20))
            plt.subplot(221)
            for difficulty_id, difficulty in enumerate(('easy', 'moderate', 'hard')):
                mask = ret['recall'][class_id, difficulty_id, min_overlap_id] > 0
                plt.plot(
                    ret['recall'][class_id, difficulty_id, min_overlap_id][mask],
                    ret['precision'][class_id, difficulty_id, min_overlap_id][mask],
                    marker=marker_style_dict[difficulty],
                    label=f'{class_id}-{difficulty}',
                )
            plt.xlim(0, 1)
            plt.ylim(0, 1)
            plt.legend()
            ...
            for difficulty_id, difficulty in enumerate(('easy', 'moderate', 'hard')):
                ...
                plt.plot(
                    ret['recall'][class_id, difficulty_id, min_overlap_id][mask],
                    ret['tp'][class_id, difficulty_id, min_overlap_id][mask],
                    marker=marker_style_dict[difficulty],
                    label=f'{class_id}-{difficulty}-TP',
                )
            plt.xlim(0, 1)
            plt.ylim(0, 12000)
            # if np.max(ret['tp'][class_id, :, :]) > 0:
            #     plt.yscale('log')
            plt.legend()
            plt.title("TP")
            plt.subplot(324)
            for difficulty_id, difficulty in enumerate(('easy', 'moderate', 'hard')):
                ...
                plt.plot(
                    ret['recall'][class_id, difficulty_id, min_overlap_id][mask],
                    ret['fp'][class_id, difficulty_id, min_overlap_id][mask],
                    marker=marker_style_dict[difficulty],
                    label=f'{class_id}-{difficulty}-FP',
                )
            plt.xlim(0, 1)
            plt.ylim(0, 12000)
            # if np.max(ret['fp'][class_id, :, :]) > 0:
            #     plt.yscale('log')
            plt.legend()
            plt.title("FP")
            plt.subplot(326)
            for difficulty_id, difficulty in enumerate(('easy', 'moderate', 'hard')):
                ...
                plt.plot(
                    ret['recall'][class_id, difficulty_id, min_overlap_id][mask],
                    ret['fn'][class_id, difficulty_id, min_overlap_id][mask],
                    marker=marker_style_dict[difficulty],
                    label=f'{class_id}-{difficulty}-FN',
                )
            plt.xlim(0, 1)
            plt.ylim(0, 12000)
            # if np.max(ret['fn'][class_id, :, :]) > 0:
            #     plt.yscale('log')
            plt.legend()
            plt.title("FN")
            plt.subplot(223)  
            for difficulty_id, difficulty in enumerate(('easy', 'moderate', 'hard')):
                ...
                plt.plot(
                    ret['recall'][class_id, difficulty_id, min_overlap_id][mask],
                    ret['thresh'][class_id, difficulty_id, min_overlap_id][mask],
                    marker=marker_style_dict[difficulty],
                    label=f'{class_id}-{difficulty}',
                )
            plt.xlim(0, 1)
