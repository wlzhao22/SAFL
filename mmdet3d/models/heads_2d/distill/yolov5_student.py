...
class YOLOv5HeadStu(nn.Module):
    ...
    def __init__(self, img_shape, in_channels, num_class, anchors,bins=8, with_ref=True, overlap = 25/180.0 * np.pi,with_ddd=False, bbox_size_anchor=None, position=None, label_smooth=True, deta=0.01, anchor_t=4, anchor_masks=[[6, 7, 8], [3, 4, 5], [0, 1, 2]], downsample_ratios=[32, 16, 8], conf_balances=[0.4,1,4], nms_thr=0.5, scores_thr=0.1, merge=False):
        super(YOLOv5HeadStu, self).__init__()
    ...
    def forward_train(self, inputs, outputs):
        ...
        if self.with_ref:
            outputs[OUTPUT_DETECTION_YOLO_BOX_LIST_REF] = self.forward_single(outputs[OUTPUT_STUDENT_FEATURE_REF])[OUTPUT_DETECTION_YOLO_BOX_LIST]
    ...
    def loss(self, inputs, outputs):
        ...
        if self.position:
            ...
            loss[f"detect_hint_backbone_loss"] = hint_loss_bk
            loss[f"detect_hint_neck_loss"] = hint_loss_nk
        return loss
    ...
    def hint_loss(self, inputs, outputs):
        ...
        teacher_features_nk = outputs[OUTPUT_TEACHER_FEATURE_AFFINE]
        student_features_nk = outputs[OUTPUT_STUDENT_FEATURE_AFFINE]

        teacher_features_bk = outputs[OUTPUT_TEACHER_BACKBONE_AFFINE]
        student_features_bk = outputs[OUTPUT_STUDENT_BACKBONE_AFFINE]
