...
class FreeSpaceHeadTea(nn.Module, metaclass=ABCMeta):
    def __init__(self,
                 layers=[1,1,1,1],
                 planes=32,
                 inplanes=96,
                 spp_planes=64,
                 head_planes=32,):
        super(FreeSpaceHeadTea, self).__init__()
    ...
    def forward_teaching(self, outputs):
        features = outputs[OUTPUT_TEACHER_BACKBONE]
        outputs[OUTPUT_TEACHER_PRED_FREESPACE] = self.forward_single(features,deploy=False)[OUTPUT_FREE_SPACE].squeeze(1)
        
        return outputs

    def forward_train(self, inputs, outputs):
        outputs.update(self.forward_single(outputs[OUTPUT_TEACHER_BACKBONE], deploy=False))
        loss_dict = {}
