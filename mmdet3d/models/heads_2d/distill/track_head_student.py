...
class TrackHeadStu(nn.Module):
    def __init__(self, in_channels, num_ids, feature_dim=128, positive_iou_threshold=0.7, negative_iou_threshold=0.3, down_ratio=8, num_cls=6):
        super(TrackHeadStu, self).__init__()
    ...
    def forward_single(self, embeddings):
        outputs = {}
        outputs[OUTPUT_STUDENT_TRACK_EMBEDDING] = self.embedding_head(embeddings[0])
    ...
    def forward_train(self, inputs, outputs):
        outputs.update(self.forward_single(outputs[OUTPUT_STUDENT_FEATURE_AFFINE]))
        outputs[OUTPUT_STUDENT_TRACK_EMBEDDING_REF] = self.forward_single(outputs[OUTPUT_STUDENT_FEATURE_REF])[OUTPUT_STUDENT_TRACK_EMBEDDING]
    ...
    def loss(self, inputs, outputs):
        ...
        bbox_preds, embedding_preds = outputs[OUTPUT_DETECTION_YOLO_BOX_LIST].detach(), outputs[OUTPUT_STUDENT_TRACK_EMBEDDING]
        ...
        bbox_preds_ref, embedding_preds_ref = outputs[OUTPUT_DETECTION_YOLO_BOX_LIST_REF].detach(), outputs[OUTPUT_STUDENT_TRACK_EMBEDDING_REF]
        ...
        loss_dict[f"track_hint_backbone_loss"] = hint_loss_bk
        loss_dict[f"track_hint_neck_loss"] = hint_loss_nk
        return loss_dict
    ...
    def hint_loss(self, inputs, outputs):
        ...
        teacher_features_nk = outputs[OUTPUT_TEACHER_FEATURE_REF]
        student_features_nk = outputs[OUTPUT_STUDENT_FEATURE_REF]

        teacher_features_bk = outputs[OUTPUT_TEACHER_BACKBONE_REF]
        student_features_bk = outputs[OUTPUT_STUDENT_BACKBONE_REF]
