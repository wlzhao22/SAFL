...
class YOLOv5Head(nn.Module):
    ...
    def __init__(self, img_shape, in_channels, num_class, anchors, with_ref=False, in_channels_backbone=[48, 96, 128, 1024], bins=8, overlap = 25/180.0 * np.pi,with_ddd=False, bbox_size_anchor=None, position=None, label_smooth=True, deta=0.01, anchor_t=4, anchor_masks=[[6, 7, 8], [3, 4, 5], [0, 1, 2]], downsample_ratios=[32, 16, 8], conf_balances=[0.4,1,4], nms_thr=0.5, scores_thr=0.1, merge=False):
    ...
    def forward_train(self, inputs, outputs):
        ...
        # outputs.update(self.forward_single(outputs[OUTPUT_FEATURE_AFFINE]))
        if self.with_ref:
            outputs[OUTPUT_DETECTION_YOLO_BOX_LIST_REF] = self.forward_single(outputs[OUTPUT_FEATURE_REF])[OUTPUT_DETECTION_YOLO_BOX_LIST]
    ...
    def loss_single(self, pred, indices, tbox, tcls, anchors, tshort, tsize, tsize_anch, tface_cls, trotation_cls, trotation_res, tshort_anch, conf_balances):
        ...
        # return lbox, lbox, lbox, lbox, lbox, lbox, lbox, lbox
    ...
    def to_kitti_2d_format(self, results):
        ret = [[] for _ in range(self.num_classes)]
        for obj in results: 
            ret[obj['label']].append([*obj['bbox'], obj['score']])
        for label in range(self.num_classes):
            ret[label] = torch.tensor(ret[label]).reshape((-1, 5))
        return ret 
