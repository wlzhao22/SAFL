...
class DLASeg(nn.Module):
    def __init__(self, base_name, pretrained, down_ratio, last_level, in_channels=3, use_dcn=True, return_single_layer=True):
        ...
        self.return_single_layer = return_single_layer

    def forward(self, x):
        # x: list of features with stride = 1, 2, 4, 8, 16, 32
        x = self.base(x)
        x: List[Tensor] = self.dla_up(x)

        if not self.return_single_layer:
            return x
