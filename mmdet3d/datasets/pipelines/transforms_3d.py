...
from .transforms_2d import RandomCrop, RandomFlip
...
class RandomFlip3D(RandomFlip):
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        if 'calib' in input_dict and 'img_shape' in input_dict:
            assert self.sync_2d is True and direction == 'horizontal'
            w = input_dict['img_shape'][1]
            calib = input_dict['calib'].copy()
            P2 = calib['P2'].copy()
            P2[0, 2] = w - P2[0, 2] - 1
            P2[0, 3] = - P2[0, 3]
            calib['P2'] = P2
...
@PIPELINES.register_module()
class RandomImageCrop3D(RandomCrop):
    ...
    def __init__(self,
                 ...
                 center_crop=False,
                 thresh=1.):
        super().__init__(crop_size, crop_type=crop_type, allow_negative_crop=allow_negative_crop, bbox_clip_border=bbox_clip_border, center_crop=center_crop, thresh=thresh)
        self.bbox2bbox3d = {
            'gt_bboxes_2d': 'gt_bboxes_3d'
        }
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('bbox_fields', []):
            ...
            # bbox3d fields
            bbox3d_key = self.bbox2bbox3d.get(key)
            if bbox3d_key in results: 
                results[bbox3d_key] = results[bbox3d_key][valid_inds]
...
@PIPELINES.register_module()
class RandomCropResize3D(RandomImageCrop3D):
    def __init__(self, 
        crop_size, crop_type='absolute', allow_negative_crop=False, bbox_clip_border=True, center_crop=False, thresh=1.,
        backend='cv2', keep_ratio=True
    ):
        super().__init__(crop_size, crop_type=crop_type, allow_negative_crop=allow_negative_crop, bbox_clip_border=bbox_clip_border, center_crop=center_crop, thresh=thresh)
    ...
    def __call__(self, results):        
        ...
        assert 'scale' not in results \
            and 'scale_factor' not in results, \
            "Do not scale more than once"

        results = super().__call__(results)
        cropped = results['crop']

        if not cropped:  #  we do not need scale when no crop is applied
            img_shape = np.asarray(results['img_shape'])
            results['scale'] = tuple(img_shape[:2])[::-1]
    ...
    def __call__(self, input_dict):
        ...
        Returns:
            dict: Results after filtering, 'gt_bboxes_3d', 'gt_labels_3d' \
                keys are updated in the result dict.
        """
        gt_labels_3d = input_dict['gt_labels_3d' if 'gt_labels_3d' in input_dict else 'gt_labels']
        gt_bboxes_mask = np.array([n in self.labels for n in gt_labels_3d],
                                  dtype=np.bool_)
        for key in ('gt_bboxes_3d', 'gt_labels_3d', 'gt_bboxes_2d', 'gt_labels', 'gt_occluded'):
...
@PIPELINES.register_module()
class BoxCenterOffset(object):
    def __init__(self, relative_offset):
        self.offset = np.asarray(relative_offset)
    
    def __call__(self, results):
        for key in results['bbox3d_fields']:
            tensor = results[key].tensor
            tensor[:, :3] += tensor[:, 3:6] * self.offset
        return results


@PIPELINES.register_module()
class RenderLidarImage(object):
    def __init__(self, mean=0., std=1., filter_size=(3, 7), no_rgb=False):
        self.mean = mean
        self.std = std 
    ...
    def _fill(self, image, mask):
        filter_size = self.filter_size
        image_blur = cv.blur(image, filter_size)
        mask_blur = cv.blur(mask, filter_size)
        image_fill = image_blur / (mask_blur[..., np.newaxis] + self.eps)  
        image_fill = image * mask[..., np.newaxis] + image_fill * (1 - mask)[..., np.newaxis]
        mask_blur = mask_blur / (mask_blur + self.eps)
        return image_fill, mask_blur
    
    def _to_img(self, points, results):
        img = results['img']
        calib = results['calib']
        ...
        points_cam = points @ (calib['R0_rect'] @ calib['Tr_velo_to_cam']).T
        points_cam = points_cam[:, :4] / points_cam[:, [3]]
        points_cam = points_cam[points_cam[:, 2] > 0]
        points_proj = points_cam @ (calib['P2']).T
        points_proj = points_proj[:, :2] / points_proj[:, [2]]
        points_proj[:, 0] = np.clip(points_proj[:, 0], 0, width - 1).astype(np.int)
        points_proj[:, 1] = np.clip(points_proj[:, 1], 0, height - 1).astype(np.int)
        indices = points_proj[:, 1] * width + points_proj[:, 0]
        _, selection_indices = np.unique(indices, return_index=True)
        indices = indices[selection_indices]
        y = (indices // width).astype(np.int)
        x = (indices % width).astype(np.int)
        canvas[y, x, :] = (points_cam[selection_indices, :3] - self.mean) / self.std 
        mask[y, x] = 1
        canvas, mask = self._fill(canvas, mask)
        img = np.concatenate((img, mask[..., np.newaxis], canvas), axis=-1) if not self.no_rgb else \
            np.concatenate((mask[..., np.newaxis], canvas), axis=-1)
        results['img'] = img.astype(np.float32) 
        return results
    
    def __call__(self, results):
        results = self._to_img(results['points'].tensor, results)
        return results
