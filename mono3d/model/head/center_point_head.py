...
from mmdet3d.models.utils.utils_3d import clip_sigmoid
from mmdet3d.ops.iou3d.iou3d_utils import nms_gpu
from mmdet3d.models.dense_heads.centerpoint_head import SeparateHead, DCNSeparateHead
from mmdet3d.core.bbox.structures.lidar_box3d import LiDARInstance3DBoxes 
from mmdet.core import build_bbox_coder, multi_apply

from ..layers.utils import nms_hm


@HEADS.register_module()
class SeparateHeadWithUncertainty(BaseModule):
    ...
    def __init__(self,
        ...
        super(SeparateHeadWithUncertainty, self).__init__(init_cfg=init_cfg)
        self.heads = heads
    ...
    def init_weights(self):
        ...
        for head in self.heads:
            if head == 'heatmap':
                self.__getattr__(head)[-1].weight.data.zero_()
                self.__getattr__(head)[-1].bias.data.fill_(self.init_bias)
            elif head == 'uncertainty':
                self.__getattr__(head)[-1].weight.data.zero_()
                self.__getattr__(head)[-1].bias.data.fill_(5)
...
@HEADS.register_module()
class CenterPointHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 bbox_coder=None,
                 common_heads=dict(),
                 loss_cls=dict(type='GaussianFocalLossWithUncertainty', reduction='mean'),
                 loss_bbox=dict(
        ...
        self.cls_uncertainty = 'WithUncertainty' in loss_cls['type']
        ...
        for num_cls in num_classes:
            ...
            heads.update(dict(uncertainty=(1, num_heatmap_convs)))
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if not self.cls_uncertainty:
                ...
                loss_heatmap = self.loss_cls(
                    (preds_dict[0]['heatmap'], preds_dict[0]['uncertainty']), 
                    heatmaps[task_id],
                    avg_factor=max(num_pos, 1),
                )
        ...
        #     heatmap_obj = torch.cat(heatmaps, axis=1).max(axis=1)[0].unsqueeze(1)
        #     act_heatmap = kwargs['activation']
        #     assert 0 <= act_heatmap.min() <= act_heatmap.max() <= 1
        #     l_act = self.loss_cls(act_heatmap.unsqueeze(1), heatmap_obj, avg_factor=sum([len(it) for it in gt_bboxes_3d]) + 1e-4)
        #     loss_dict['loss_act'] = l_act
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # max pooling as nms 
            # batch_heatmap = F.avg_pool2d(batch_heatmap, 3, 1, 1)
            batch_heatmap = nms_hm(batch_heatmap)
