...
from mono3d.ops.packnet import PackLayerConv2d, PackLayerConv3d, UnpackLayerConv3d

BN_MOMENTUM = 0.1


@BACKBONES.register_module()
class CustomDLASegWithPackNet(nn.Module):
    def __init__(self, base_name, pretrained, down_ratio, last_level, in_channels=3, use_dcn=False, dropblock_cfg=None):
        super(CustomDLASegWithPackNet, self).__init__()
        assert down_ratio in [1, 2, 4, 8, 16]
        assert use_dcn == False, "we do not support dcn since we use packlayer instead."
        
        self.in_channels = in_channels
        self.first_level = int(np.log2(down_ratio))
        self.last_level = last_level
        self.base = globals()[base_name](pretrained=pretrained, in_channels=in_channels)
        ...
        if dropblock_cfg is not None: 
            self.enable_dropblock = True 
            self.dropblock = nn.ModuleList([DropBlock2D(**cfg) for cfg in dropblock_cfg])
        else:
            self.enable_dropblock = False 
                            
    def forward(self, x):
        # x: list of features with stride = 1, 2, 4, 8, 16, 32
        base_features = self.base(x)
        x: List[Tensor] = self.dla_up(base_features)

        y = []
        for i in range(self.last_level - self.first_level):
            y.append(x[i].clone())
        if self.enable_dropblock:
            y = [self.dropblock[i](y[i]) for i in range(len(y))]
        self.ida_up(y, 0, len(y))

        return [*base_features[:self.first_level], y[-1]]
...
class DLA(nn.Module):
    def __init__(self, levels, channels, in_channels=3, num_classes=1000,
    ...
    def forward(self, x):
        y = []
        x = self.base_layer(x) if self.in_channels == 3 else self.base_layer2(x)
        for i in range(6):
    ...
    def load_pretrained_model(self, data='imagenet', name='dla34', hash='ba72cf86'):
        ...
        load_state_dict(self, model_weights, strict=False)
    ...
    def __init__(self, chi, cho, use_dcn=True):
        ...
        if use_dcn:
            ...
            self.conv = nn.Sequential(
                PackLayerConv2d(chi, kernel_size=3, r=1), 
                nn.Conv2d(chi, cho, 1), 
            )
...
class IDAUp(nn.Module):
    ...
    def __init__(self, o, channels, up_f, use_dcn=True):
        ...
        for i in range(1, len(channels)):
            ...
            #                         groups=o, bias=False)
            up = UnpackLayerConv3d(o, o, f * 2 + 1, r=f, d=8)
