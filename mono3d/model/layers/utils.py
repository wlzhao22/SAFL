...
def nms_hm_dilate(heatmap, kernel=3, reso=1):
    kernel = int(kernel / reso)
    if kernel % 2 == 0:
        kernel += 1
    
    pad = (kernel - 1) // 2
    hmax = F.max_pool2d(heatmap,
                        kernel_size=(kernel, kernel),
                        stride=1,
                        padding=pad)
    ...
    eq_index = F.max_pool2d(eq_index, 
        kernel_size=(kernel, kernel), 
        stride=1, 
        padding=pad
    )
    # heatmap = F.max_pool2d(heatmap, 
    #     kernel_size=(kernel, kernel), 
    #     stride=1, 
    #     padding=pad
    # )
...
def select_point_of_interest(batch, index, feature_maps):
    ...
    # [N, H, W, C] -----> [N*H*W, C]
    feature_maps = feature_maps.view(-1, channel)
    # expand index in channels
    return feature_maps[index]
